GetSizeOfGDELT <- function(historical.url.root="http://www.gdeltproject.org/data/backfiles/?F=2",
                           daily.url.root="http://www.gdeltproject.org/data/dailyupdates/?F=2") {
  ...
  hs <- gsub("<tr>(.(?!/td>))*</td>(.(?!/td>))*</td>(.(?!>))*\\\">", "", hs, perl=TRUE)
  hs <- gsub("M.*$", "", hs)
  ...
  ds <- gsub("<tr>(.(?!/td>))*</td>(.(?!/td>))*</td>(.(?!>))*\\\">", "", ds, perl=TRUE)
  ds <- gsub("M.*$", "", ds)
